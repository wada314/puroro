...
//! future `build.rs` / CLI wrappers should call [`generate`] / [`generate_from_bytes`]
//! (or the descriptor / resolved helpers) directly.
...
/// Decode a `CodeGeneratorRequest`, generate files, and return response wire bytes.
pub fn generate_from_bytes(request_bytes: &[u8]) -> Result<Vec<u8>> {
    let request = decode_request(request_bytes)?;
    let response = match generate(&request) {

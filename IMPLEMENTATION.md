...
The oneof drives each variant with the **field's own** primitives — no bespoke helpers on the enum. Build-empty (for `variant_mut`'s `make`) is `new_in(alloc)` / `new()` / `with_message_in(alloc)`. Merging one wire occurrence is the field's own idiom: `field.bind_oneof(common).merge(wire, buf)` for LEN/VARINT — `bind_oneof` is the presence-free bind for oneof variants (a oneof has no bit; `Implicit` presence ignores the one it is handed, so it binds against a dummy index and yields the same `…FieldMut` view) — and `field.merge(common, wire, buf)` for the message variant (which needs no bind and merges *into* the present child). Read/read-mut are `value()`/`value_mut(…)` (LEN/VARINT) or `get()`/`get_present_mut()` (message), presence-agnostic. The VARINT variant owns no heap, so its `OneofDeallocate` arm is a no-op and its constructor ignores the allocator; the message variant is always built with the child present, so its accessors `unwrap()` the child.
...
- `variant_mut(is_match, make) -> &mut E` — keeps the active variant if `is_match`, else frees it and installs `make(alloc.clone())`. Backs both the per-variant `_mut` accessors and the decode arms; each pattern-matches out the inner field wrapper and then either returns the kind-appropriate handle (`value_mut(alloc)` for LEN, `value_mut()` for VARINT, `get_present_mut().unwrap()` for message) or merges into it (`bind_oneof(common).merge(…)` / `merge(common, …)`).

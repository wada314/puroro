...
**Bound-view mutation (`bind`).** For the LEN field families the mutation methods are not called on the field directly. Instead, mirroring the way `unmanaged` reunites a split value with its allocator through a temporary guard, the field is first *bound* to the message common state: `field.bind(&mut self._common, bit)` returns a short-lived view (`SingularLenFieldMut` / `RepeatedLenFieldMut`) that carries `(field, common, bit)` together, and the actual operation (`value_mut` / `merge` / `clear` / `push_in`) is a consuming method on that view. This keeps the field struct a pure storage holder, removes the repeated threading of `common`/`bit`/allocator, and collapses each generated accessor to a single call — e.g. `self.owner_id.bind(&mut self._common, Self::BIT_OWNER_ID).value_mut()`. The view carries two lifetimes so the returned guard borrows only the field storage; the `common` borrow is released as the call returns. Read-only paths (`value` / `optional` / `has` / `encoded_len` / `encode_raw`) need only a shared `&common` and stay as plain field methods.

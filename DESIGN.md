...
| message | `NestedMessageField::with_message_in(alloc)` | `field.merge(common, wire, buf)` | `get()` / `get_present_mut()` |

`bind_oneof(common)` is the presence-free bind for oneof variants: a oneof carries no presence bit (the enclosing `OneofSlot` tracks which variant is set), and the variant's `Implicit` policy ignores the bit, so it binds against a dummy index and returns the *same* `…FieldMut` view as an ordinary `bind`. The message variant needs no such bind — `NestedMessageField::merge` already takes `common` directly, mirroring how ordinary message fields merge (and merging *into* the present child rather than replacing it).

The VARINT variant owns no heap: its `OneofDeallocate` arm is a no-op, and its `make` ignores the allocator. The message variant is always constructed with the child present, so its read/mut accessors `unwrap()` the child.

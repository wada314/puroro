...
where
    ...
    L: LabelWrappedType<VariantNativeType<X, V>>,
    Self: FieldTypeGen<
        X,
        L,
        tags::wire::Variant<V>,
        Type = <L as LabelWrappedType<VariantNativeType<X, V>>>::Type,
    >,
{
...
where
    ...
    // ..and the type above can be wrapped by Option<> or Vec<> according to
    // the label (this is always true),
    L: LabelWrappedType<Bits64NativeType<X, V>>,
    // ..and the Rust type generated by FieldTypeGen is equal to the type above.
    Self: FieldTypeGen<
        X,
        L,
...
where
    ...
    Self: FieldTypeGen<X, L, tags::Bytes, Type = <(X, L) as LabelWrappedLdType<[u8]>>::Type>,
{
    fn deser_from_scoped_bytes_iter<I>(

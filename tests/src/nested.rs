    ...
    >(
        ...
        while let Some((number, field_data))
            ...
            let result: self::_puroro::Result<()> = (|| {
                match number {
                    1i32 => {
                        let view_ref: &mut self::_root::nested::_view::MsgView = &mut self
                            .0;
                        self::_pinternal::FieldType::deser_from_field_data(
                            &mut view_ref.fields.item_outer,
...
impl ::std::default::Default for self::Msg {
    fn default() -> Self {
        Self(
            ::std::boxed::Box::new(
                <self::_root::nested::_view::MsgView as ::std::default::Default>::default(),
            ),
        )
    }
}
...
where
    ...
    self::_root::nested::_view::MsgView: self::_puroro::DefaultIn<A>,
{
    fn default_in(allocator: A) -> Self {
        Self(
            ::std::boxed::Box::new_in(
                <self::_root::nested::_view::MsgView as self::_puroro::DefaultIn<
                    A,
    ...
    impl self::_pinternal::MessageViewInternal for self::MsgView {
        ...
        fn new_boxed_in<A: ::std::alloc::Allocator>(
            ...
            let (fields, allocator) = {
                let (item_outer, _allocator) = self::_pinternal::FieldType::new_in(
                    shared.bitfield_mut(),
                    _allocator,
                );
                (
                    self::_root::nested::_fields::MsgFields {
                        item_outer,
                    },
                    _allocator,
                )
